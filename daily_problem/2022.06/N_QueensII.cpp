...
public:
  ...
  bool check(vector<vector<bool>> &board, int row, int col) {
    ...
    for (int i = 0; i <= row; i++) {
      if (board[i][col])
        return false; // checking if any queen already placed on same column
                      // previously
  ...
  int solve(vector<vector<bool>> &board, int row) {
    ...
    for (int col = 0; col < size(board); col++)
      if (check(board, row, col)) { // check if we can place at (row, col)
        ...
        count += solve(
            board,
            row + 1); // explore for the next row. The function will return 1 if
                      // all N queens get placed for current combination
        board[row][col] = false; // backtrack - remove previously placed queen
                                 // and try for different columns
      }
    return count;
  }
};